...
describe('mapRowsToDepths', () => {
  test('maps one row to one depth', () => {
    ...
    expect(mappedRows.depth30Row).toHaveProperty('forecast_type_id', 1)
    expect(mappedRows.depth60Row).toHaveProperty('forecast_type_id', 2)
    expect(mappedRows.depth90Row).toHaveProperty('forecast_type_id', 3)
    expect(mappedRows.depthAverageRow).toHaveProperty('forecast_type_id', 4)
  })
